...
def adjacent_tile(tile, dx, dy, tiles):
    ...
    try:
        tileid, image = tiles.get_tile_by_id(other)
        return image.data.numpy()
    except KeyError:
        return None


def buffer_tile_image(tile, tiles, overlap, tile_size, bands=4):
    ...
    img = np.zeros((bands, ts + oo, ts + oo)).astype(np.float32)

    # fmt:off
    img[:, 0:o,        0:o       ] = ul[:, -o:ts, -o:ts] if ul is not None else np.zeros((bands, o,   o)).astype(np.float32)
    img[:, 0:o,        o:ts+o    ] = uc[:, -o:ts,  0:ts] if uc is not None else np.zeros((bands, o,  ts)).astype(np.float32)
    img[:, 0:o,        ts+o:ts+oo] = ur[:, -o:ts,   0:o] if ur is not None else np.zeros((bands, o,   o)).astype(np.float32)
    img[:, o:ts+o,     0:o       ] = cl[:, 0:ts,  -o:ts] if cl is not None else np.zeros((bands, ts,  o)).astype(np.float32)
    img[:, o:ts+o,     o:ts+o    ] = cc                  if cc is not None else np.zeros((bands, ts, ts)).astype(np.float32)
    img[:, o:ts+o,     ts+o:ts+oo] = cr[:, 0:ts,    0:o] if cr is not None else np.zeros((bands, ts,  o)).astype(np.float32)
    img[:, ts+o:ts+oo, 0:o       ] = bl[:, 0:o,   -o:ts] if bl is not None else np.zeros((bands, o,   o)).astype(np.float32)
    img[:, ts+o:ts+oo, o:ts+o    ] = bc[:, 0:o,    0:ts] if bc is not None else np.zeros((bands, o,  ts)).astype(np.float32)
    img[:, ts+o:ts+oo, ts+o:ts+oo] = br[:, 0:o,     0:o] if br is not None else np.zeros((bands, o,   o)).astype(np.float32)
    # fmt:on
